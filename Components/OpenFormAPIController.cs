...
namespace Satrabel.OpenForm.Components
{
    ...
    public class OpenFormAPIController : DnnApiController
    {
                    ...
                    if (settings != null && settings.Notifications != null)
                    {
                        foreach (var notification in settings.Notifications)
                        {
                            try
                            {
                                ...
                                string send = SendMail(from.ToString(), to.ToString(), (reply == null ? "" : reply.ToString()), notification.EmailSubject, body, attachements);
        ...
        private string SendMail(string mailFrom, string mailTo, string replyTo, string subject, string body, List<Attachment> attachments = null)
        {
            //string mailFrom
            //string mailTo, 
            string cc = "";
